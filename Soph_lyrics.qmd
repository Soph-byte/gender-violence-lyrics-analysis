...
```{r, message=FALSE}
library(rvest)     # For HTML parsing (Web Scraping)
library(stringdist) # For computing string distances 
library(httr)      # For handling API requests
library(jsonlite)  # For parsing JSON data from APIs
library(dplyr)     # For data manipulation (tidy data processing)
library(purrr)     # For functional programming (map functions)
library(tibble)    # For modern data frames (tibbles)
library(tidyr)     # For handling nested data structures
library(stringr)   # For working with strings and regular expressions
library(stopwords) # For working with stopwords (text cleaning)
library(widyr)     # For computing word co-occurrences (text mining)
library(ggplot2)   # For data visualization (general plotting)
library(tidytext)  # For text mining and tokenization
library(quanteda)  # For advanced text analysis and NLP (natural language processing)
...
get_spotify_token <- function(client_id, client_secret) {
  ...
  response <- POST(
    auth_url,  # Send request to Spotify's authentication API
    body = list(
      grant_type = "client_credentials",  # Specify client credentials authentication
      client_id = client_id,  # Provide Client ID
      client_secret = client_secret  # Provide Client Secret
    ),
    encode = "form"  # Encode request as form data
  )
  
  # Check if authentication was successful
  if (http_status(response)$category != "Success") {  
    stop("Failed to authenticate. Check your client ID and secret.")  # Stop execution if authentication fails
  }
  
  # Parse the API response and extract the access token
  token_data <- content(response, as = "parsed", type = "application/json")  # Convert response to JSON
  
  # Ensure the response contains an access token
  if (!"access_token" %in% names(token_data)) {  
    stop("Error: Could not retrieve access token.")  # Stop execution if no token is returned
  }
...
access_token <- get_spotify_token(client_id, client_secret)  # Authenticate and retrieve access token
...
# Calculation of word correlations
word_correlation <- german_lyrics_cleaned %>%
  unnest_tokens(word, lyrics) %>%
  pairwise_cor(word, track_name, sort = TRUE)

# Define a list of masculine and feminine terms
male_terms <- c("digga")
female_terms <- c("frau'n")

# Extract words that correlate with masculine terms
...
# Extract words that correlate with feminine terms
...
sentiment_analysis_nrc_es <- top_artists_tracks_es_atomic %>%
  ...
  count(sentiment, sort = TRUE)  # Count occurrences of each emotion
...
# Step 1: Get Spanish stopwords
spanish_stopwords <- stopwords::stopwords("es")

# Step 2: Process lyrics and compute word correlations
...
female_related_words <- word_correlation_es %>%
  ...
  filter(correlation > 0.1) %>%
  arrange(desc(correlation)) %>%
  head(20)  # Focus on top 20 for clarity

# Step 5: Create a clean visualization
ggplot(female_related_words, 
       aes(x = reorder(other_term, correlation), y = correlation)) +
...
# Perform sentiment analysis on Spanish lyrics
